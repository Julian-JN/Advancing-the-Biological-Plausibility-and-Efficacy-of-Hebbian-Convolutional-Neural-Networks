...
from models.model_depthwise import Net_Depthwise
...
"""
Identical to experiment_hebbian.py, but adapted for depthwise convolutions
"""
...
if __name__ == "__main__":
    ...
    wandb_logger = Logger(
        f"Best_Depthwise_SoftHebb",project='CIFAR10_Dataset', model=model)
    ...
    plot_ltp_ltd(model.conv_point3, 'conv_point3', num_filters=10, detailed_mode=True)
    ...
    visualize_mapped_importance(model, tst_set, model.conv_point2, max_batch_images=8)
    visualize_mapped_importance(model, tst_set, model.conv3, max_batch_images=8)
    visualize_mapped_importance(model, tst_set, model.conv_point3, max_batch_images=8)
    clear_image_cache()
    ...
    visualize_top_activations(model, tst_set, model.conv_point2, num_top=25, max_batch_images=8)
    visualize_top_activations(model, tst_set, model.conv3, num_top=25, max_batch_images=8)
    visualize_top_activations(model, tst_set, model.conv_point3, num_top=25, max_batch_images=8)
    clear_image_cache()
    ...
    visualize_receptive_fields_context(model, tst_set, model.conv_point2, num_filters=25, max_batch_images=8)
    visualize_receptive_fields_context(model, tst_set, model.conv3, num_filters=25, max_batch_images=8)
    visualize_receptive_fields_context(model, tst_set, model.conv_point3, num_filters=25, max_batch_images=8)
    clear_image_cache()
